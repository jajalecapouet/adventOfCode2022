...
class Sim
{
	...
	int _checkAround()	const
	{
		...
		if (_g[_Gilberte.second][_Gilberte.first - 1] == EMPTY)
			toReturn |= 4;
		if (_g[_Gilberte.second + 1][_Gilberte.first] == EMPTY)
			toReturn |= 2;
...
public :
	...
	Sim &operator=(const Sim &o)
	{
		...
		_startPoint = o._startPoint;
		_endPoint = o._endPoint;
		_turn = o._turn;
		_g = o._g;
		return *this;
	}

	void	dnf(unsigned int &result, visited &v) const
	{
		if (_isKnown(v))
			return;
