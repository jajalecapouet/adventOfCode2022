...
int main(int ac, char **av)
{
	...
	coord	end = findGoal(grid);
	int min = -1;
	for (int it = 0; it < grid.size(); ++it)
	{
		for (int it2 = 0; it2 < grid[it].size() ; ++it2)
		{
			if (grid[it][it2] == 1)
			{
				coord start = std::make_pair(it2, it);
				AStar	result(grid, start, end);
				int count = result.stepCount();
				if (min == -1)
					min = count;
				else if (count != -1)
